...
if not df_filtered.empty:
    ...
    fig_line = px.line(df_filtered, x=df_filtered.index, y=df_filtered.columns, title="Evolução Mensal das Contas")
    ...
    if totais_por_conta.dropna().empty:
        ...
        fig_bar = px.bar(totais_por_conta, x=totais_por_conta.index, y=totais_por_conta.values, title="Total por Conta Contábil no Ano")
    ...
    if valores_mes.empty:
        ...
        fig_pie = px.pie(names=valores_mes.index, values=valores_mes.values, title=f"Composição Percentual - {selected_month}")
...
dashboard_url = "https://seuapp.streamlit.app/"
